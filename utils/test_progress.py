...
class ProgressMonitorTest(TestCase):

    def test_construct(self):
        logger = Mock()
        pl = ProgressMonitor(logger)
    ...
    def test_count_must_be_a_positive_integer(self):
        ...
        with self.assertRaisesRegex(ValueError, 'positive integer'):
            ProgressMonitor(logger, count=0)


    def test_logger_is_called_every_count_calls(self):
        logger = Mock()
        pl = ProgressMonitor(logger, count=2)
        pl.info('1')
        pl.info('2')
        pl.info('3')
        pl.info('4')
    ...
    def test_delay_must_be_positive(self):
        ...
        with self.assertRaisesRegex(ValueError, 'must be >= 0'):
            ProgressMonitor(logger, delay=-1)
    ...
    def test_logger_is_called_after_delay(self):
        ...
        with patch('progress.time', autospec=True) as clock:
            ...
            pl = ProgressMonitor(logger, delay=1)
            pl.info('1')
            pl.info('2')
